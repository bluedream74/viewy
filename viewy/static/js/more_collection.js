...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadNextPost() {
    ...
    const collectionId = collectionDataElement.dataset.collectionId;
    console.log("Collection ID:", collectionId);


    const csrftoken = getCookie('csrftoken'); // CSRFトークンを取得

    fetch(`/posts/get_more_collection/`, { //次の投稿を読み込むビューに送信！
      method: 'POST', // メソッドをPOSTに変更
      body: `last_post_id=${lastPostId}&collection_id=${collectionId}`, // collection_idをここに追加
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadPreviousPost() {
      ...
      fetch(`/posts/get_more_previous_collection/`, {
        method: 'POST',
        body: data,
        credentials: 'include',
        headers: {
