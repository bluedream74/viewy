...
document.addEventListener('DOMContentLoaded', function () {
  function handleBookScroll(event) {
    ...
    const totalPageDisplay = book.parentNode.querySelector('.total-page');
    const pageWidth = mangaPages[0].offsetWidth;
    let currentPage = 1 - Math.floor(book.scrollLeft / pageWidth);
    const maxPages = parseInt(totalPageDisplay.textContent, 10);

    // もしカレントページが最大ページ数を超えていれば、カレントページを最大ページ数にセット
