...
class AgeVerificationMiddleware:
    ...
    def __call__(self, request):
        ...
        if not any(request.path.startswith(url) for url in excluded_urls):
            ...
            if request.path != age_verification_url and not request.COOKIES.get('is_over_18') and request.path != settings.LOGIN_API_URL and request.path != settings.REGISTER_API_URL:
